...
class CSCMCalibrationPipeline:
    ...
    def __init__(self, config_file, data_directory, data_name_prefix, optional_arg_dict=None):
    ...
    def prune_distribution(self, file_endstring=None):
        ...
        for varname, varinfo in self.configs["prune_configs"].items():
            prune_lists.append([varname, f"{self.configs['prior_configs']['input_dir']}{varinfo[1]}", varinfo[2]])
        prune_all_chunks(total_samples=tot_samples, prune_lists=prune_lists, num_chunks=num_chunks,file_endstring=file_endstring)
