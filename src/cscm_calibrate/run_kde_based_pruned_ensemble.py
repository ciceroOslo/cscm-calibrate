...
calibdata = pd.DataFrame(
    data={
        ...
        "Central Value": [484.82157000000063, 1.24, -1.18, 410.1, 2.9, 3.2],
        "sigma": [36.8551891022091, 0.073, 0.7, 0.4, 0.4, 0.9],
    }
)
# distnums = 500000
distrorun1 = DistributionRun(
    None, json_file_name="data/post_pruning_resample_both_10000.json"
)
    ...
    if (
        ...
        results_for_fit_dict_1d[data["Variable Name"]] = (
            ...
            - (
                results_sub.iloc[
                    :,
                    data["Yearstart_norm"] - 1750 + 7 : data["Yearend_norm"] - 1750 + 8,
                ]
            ).mean(axis=1)
        ).values
...
plot = True
if plot:
    plot_distributions_w_obs.plot_distributions(results, f"post_pruning_test1_full")
    # print(distrorun1.cfgs)
print(results.head())
# There used to be a pruning sanity check here...
store = True
if store:
    # results = results[results["variable"] == "Surface Air Ocean Blended Temperature Change"]
    # ids = results["run_id"].to_numpy(int)
    # results = results.iloc[:, 107:].to_numpy(float)
    # np.save("data/temp_1850-2023.npy", results)
    # np.save("data/sample_ids.npy", ids)
    # sys.exit(4)
    ...
    # sys.exit(4)
    store = pd.HDFStore("data/data_post_pruning.h5")
    store["targ"] = targ
