...
distnums = 6000000
chunk_size = 10000
testconfig.make_config_lists(distnums, json_fname=f"data/configs_{distnums}_.json", max_chunk_size=chunk_size)
chunk_nums = int(np.ceil(distnums/chunk_size))

for i in range(chunk_nums):
    file_midstring = f"{distnums}_chunk_{i}"
    distrorun1 = DistributionRun(testconfig, json_file_name= f"data/configs_{file_midstring}.json", numvalues= distnums)
    ...
    plot = False
    if plot:
        plot_distributions_w_obs.plot_distributions(results, f"{distnums}_test1_full")
    ...
    if store:
        ...
        ids = results["run_id"].to_numpy()
        results = results.iloc[:, 107:].to_numpy(float)
        np.save(f"data/temp_{file_midstring}_1850-2023.npy", results)
